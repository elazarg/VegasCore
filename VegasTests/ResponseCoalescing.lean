...
/-- One native policy realizes any response strategy at every reachable entry,
including histories generated by arbitrary earlier deviations. -/
theorem policy_realization (history : arena.History) (control : NativeControl runtime)
    (stateEq : history.state = some control)
    (active : runtime.nativeActor history.state = some ())
    (policy : ResponsePolicy (graph := graph) (fun _ => 3)) :
