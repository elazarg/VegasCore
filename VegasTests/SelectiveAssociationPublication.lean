...
theorem native_history_reachable (control : nativeApp.Control)
  ...
  obtain ⟨inputs, selected, reachable⟩ := nativeRuntime.reactive_history_graph_reachable
    nativeLeaks (FinDist.pure nativeInputs) nativeHorizon nativeScheduler
...
theorem native_history_correctness_le (control : nativeApp.Control)
