...
theorem evalP_eq_denote :
  ...
  | ret e =>
      ...
      simpa [denote, MT.ret] using (toMeasure_evalP_ret (W := W) (e := e) (env := env))
  | letDet e k ih =>
          ...
          have h_eval :
              (evalP (.doBind (.sample K) k) env).toMeasure =
                (K env).weights.foldr
                  ...
                  0 := by
            simpa using (toMeasure_evalP_sample (W := W) (K := K) (k := k) (env := env))
