...
This directory contains the older design-exploration collection of languages.
It is intentionally secondary to the mainline `Vegas/` tree.
It is kept for reference and comparison, not as the primary maintained surface.
...
The files `LetCore.lean`, `LetProb.lean`, `LetProtocol.lean`, `LetGames.lean`, and `GameTheory.lean` at the top level of `Explorations/` are re-export modules that import all files in the corresponding subdirectory.
