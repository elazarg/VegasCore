...
abbrev Ctx (Ty : Type) : Type := List Ty
...
inductive Var : Ctx Ty → Ty → Type where
  | vz {Γ τ} : Var (τ :: Γ) τ
  | vs {Γ τ τ'} (v : Var Γ τ) : Var (τ' :: Γ) τ
deriving Repr

/-- Weaken a variable reference to work in an extended context. -/
def Var.weaken {Γ : Ctx Ty} {τ τ' : Ty} : Var Γ τ → Var (τ' :: Γ) τ
  | .vz   => .vs .vz
  | .vs v => .vs (Var.weaken v)

@[simp] theorem Var.weaken_vz {Γ : Ctx Ty} {τ τ' : Ty} :
    Var.weaken (τ' := τ') (Var.vz : Var (τ :: Γ) τ) = Var.vs Var.vz := rfl

@[simp] theorem Var.weaken_vs {Γ : Ctx Ty} {τ τ' τ'' : Ty} (x : Var Γ τ) :
...
def Env (Val : Ty → Type) : Ctx Ty → Type
  | [] => Unit
  | τ :: Γ => Val τ × Env Val Γ
...
/-- Total lookup. -/
def get {Val : Ty → Type} {Γ : Ctx Ty} {τ : Ty} (ρ : Env Val Γ) (x : Var Γ τ) : Val τ :=
  match x, ρ with
  | .vz,   (v, _) => v
  | .vs v, (_, ρ) => get ρ v

/-- Construct an environment by consing a value. -/
def cons {Val : Ty → Type} {Γ : Ctx Ty} {τ : Ty} (v : Val τ) (ρ : Env Val Γ) : Env Val (τ :: Γ) :=
  (v, ρ)

/-- Head of the environment. -/
def head {Val : Ty → Type} {Γ : Ctx Ty} {τ : Ty} : Env Val (τ :: Γ) → Val τ
  | (v, _) => v

/-- Tail of the environment. -/
def tail {Val : Ty → Type} {Γ : Ctx Ty} {τ : Ty} : Env Val (τ :: Γ) → Env Val Γ
  | (_, ρ) => ρ

@[simp] theorem get_vz {Val : Ty → Type} {Γ : Ctx Ty} {τ : Ty} (ρ : Env Val (τ :: Γ)) :
    get ρ Var.vz = head ρ := by
  cases ρ; rfl

@[simp] theorem get_cons_vz {Val : Ty → Type} {Γ : Ctx Ty} {τ : Ty} (v : Val τ) (ρ : Env Val Γ) :
    get (cons v ρ) Var.vz = v :=
  rfl

@[simp] theorem get_cons_vs {Val : Ty → Type} {Γ : Ctx Ty} {τ τ' : Ty} (v : Val τ') (ρ : Env Val Γ) (x : Var Γ τ) :
...
@[ext] theorem ext {Val : Ty → Type} {Γ : Ctx Ty} {ρ₁ ρ₂ : Env Val Γ}
