...
state. Message inclusion consumes a handler-knowledge contract; all other
transitions share the candidate-host semantics. The release law uses one
policy-trace induction for both unrestricted and publicly validated admission.
-/
...
theorem CandidateExecutionRelated.playerStep
    ...
    (hleft : nextLeft ∈
      ((runtime.candidateHost applyMessage).playerStep who left leftCommand).support)
    (hright : nextRight ∈
      ((runtime.candidateHost applyMessage).playerStep who right rightCommand).support) :
    CandidateExecutionRelated runtime known nextLeft nextRight := by
  refine ⟨?_, ?_, ?_⟩
  · have hl : nextLeft.native ∈
        (((runtime.candidateHost applyMessage).playerStep who left leftCommand).map
          MessageInterface.PolicyExecution.native).support := by
      rw [FinDist.support_map]
      exact ⟨nextLeft, hleft, rfl⟩
    have hr : nextRight.native ∈
        (((runtime.candidateHost applyMessage).playerStep who right rightCommand).map
          MessageInterface.PolicyExecution.native).support := by
      rw [FinDist.support_map]
  ...
  · intro observer
    ...
    · subst observer
      rw [MessageApplication.playerStep_history_self (runtime.candidateHost applyMessage)
          who left leftCommand nextLeft hleft,
        MessageApplication.playerStep_history_self (runtime.candidateHost applyMessage)
          who right rightCommand nextRight hright]
      ...
      exact List.rel_append (related.histories who)
        (List.Forall₂.cons
          ...
          List.Forall₂.nil)
    · rw [MessageApplication.playerStep_other_history (runtime.candidateHost applyMessage)
        who observer hwho
        left leftCommand nextLeft hleft,
        MessageApplication.playerStep_other_history (runtime.candidateHost applyMessage)
        who observer hwho
        right rightCommand nextRight hright]
      exact related.histories observer
  · rw [MessageApplication.playerStep_environmentHistory (runtime.candidateHost applyMessage)
        who left leftCommand nextLeft hleft,
      MessageApplication.playerStep_environmentHistory (runtime.candidateHost applyMessage)
        who right rightCommand nextRight hright]
    exact related.environmentHistory

theorem CandidateExecutionRelated.environmentStep
    (related : CandidateExecutionRelated runtime known left right)
    (hknowledge : CandidateHandlerKnowledge applyMessage)
    (command : runtime.candidateApplication.EnvironmentPolicyCommand)
    (nextLeft nextRight : runtime.candidateApplication.PolicyExecution)
    (hleft : nextLeft ∈
      ((runtime.candidateHost applyMessage).environmentPolicyStep left command).support)
    (hright : nextRight ∈
      ((runtime.candidateHost applyMessage).environmentPolicyStep right command).support) :
    CandidateExecutionRelated runtime known nextLeft nextRight := by
  cases command <;>
    simp only [environmentPolicyStep, advance, EnvironmentPolicyCommand.toAction,
      MessageApplication.step, candidateHost, host, FinDist.pure_bind,
      FinDist.map_pure, FinDist.mem_support_pure] at hleft hright <;>
    subst nextLeft <;> subst nextRight
  all_goals
    refine ⟨?_, related.histories, ?_⟩
    rotate_left
    · simp only [State.environmentView, related.environmentHistory,
        related.native.pool, related.native.receipts, related.native.publicState]
  · exact related.native.deliver _ _
  · exact related.native.includePending hknowledge _
...
observation when admission respects knowledge and the paired commands respect
the disclosure boundary. There is no restriction to prepared submissions,
and no fairness assumption on the full-pool environment. -/
theorem candidate_firstRelease_observation_law {Observation : Type*}
    ...
    (related : CandidateExecutionRelated runtime known left right) :
    (((runtime.candidateHost applyMessage).tracePolicies
      leftPlayers environment schedule left).map
        (PolicyTrace.firstRelease (app := runtime.candidateHost applyMessage) release)).map
          observe =
      (((runtime.candidateHost applyMessage).tracePolicies
        rightPlayers environment schedule right).map
        (PolicyTrace.firstRelease (app := runtime.candidateHost applyMessage) release)).map
          observe := by
  apply (runtime.candidateHost applyMessage).tracePolicies_firstRelease_of_steps
    ...
    (fun _ _ h => h.environmentStep hknowledge) ?_ schedule left right related
