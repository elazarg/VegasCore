...
backend captures the EVM's strategically central feature, which is part of
why that setting motivated the design: a public mempool with front-running is
exactly pending-message visibility, which the message-in-flight machine models
and proves controllable.  A full EVM backend --- gas, storage encoding, and
contract dispatch --- or a ConCertLean adapter~\citep{GershuniConCertLean}
would discharge the same kind of refinement obligations at their own runtime
boundary.  Neither is built here; the abstract layers are evidence that the
obligations such a backend faces are exactly the ones this interface isolates.
