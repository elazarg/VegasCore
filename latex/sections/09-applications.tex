...
\item Allocation rules are public payoff expressions over the
  revealed bids; the four kernel games then expose these as
  strategic-form games whose equilibrium predicates are the standard
  incentive-compatibility, individual-rationality, and revenue
  notions.
...
The kernel-game API exposes everything an external solver needs to
compute equilibria: a strategy carrier, a profile-to-outcome
distribution kernel, and a utility.  Existence corollaries for
mixed Nash, correlated equilibrium, and coarse correlated
equilibrium follow directly from finite-game fixed-point theorems
\citep{Nash1951,Aumann1974}.  External equilibrium-computation
tools (e.g.\ Gambit~\citep{McKelveyMcLennanTurocy2006Gambit},
PRISM-games~\citep{KwiatkowskaNormanParkerSantos2020PRISMGames})
can be driven by exporting either the kernel game or the
canonical EFG presentation; the correctness of such an export is a
checked theorem about the bridge, not an after-the-fact
sanity check.
