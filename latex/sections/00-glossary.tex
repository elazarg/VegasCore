...
This report develops a mechanized semantics for a finite probabilistic
...
behavioral strategies, Nash equilibrium) but is not assumed to know
...
\item[Kuhn's theorem (finite realization and deviation form).]
  ...
  outcome distribution as some behavioral strategy that randomizes
  locally at each information set~\citep{Kuhn1953}.  The classical
  statement requires \emph{perfect recall}.  In this formalization, the
  public checked-program statement is a native completed-frontier
  theorem.  Behavioral frontier profiles, product mixed pure frontier
  profiles, and pure frontier profiles are packaged into an
  outcome-level realization result and a strategy/deviation-level
  equivalence: unilateral deviations in one frontier presentation can be
  matched by unilateral deviations in the other with the same completed
  outcome law.  Nash transport is a downstream use of this package, not
  the primary statement.  FOSG and EFG bridges provide payoff-faithful
  presentations of the same frontier semantics; they are not the
  reduction used to prove the checked-program Kuhn statement.
...
\item[Refinement / forward simulation.]
  ...
  specification by any externally observable behavior.  Conventionally
...
For a top-down map from the semantic picture to file paths, see the
supplement.  Cross-references in this report use the current Lean module
and theorem names.  The supplement records the definition shapes and
paper-facing theorem index used by the mechanization.
