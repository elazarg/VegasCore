...
to walk a specific schedule must recover that schedule explicitly
(e.g.\ via the source-order linear read of supplement~\S7.4); the
cost is small in practice.
...
takes a normalisation proof, with no measure-theoretic continuation.
...
the frontier-stability proof.  Nullable yields are the present
...
\paragraph{Native/FOSG bridge not yet discharged.}  The Vegas-facing
pure and PMF-behavioural strategy carriers are now native round-view
carriers over the event-graph machine.  The bounded FOSG view remains
the presentation used for the generic FOSG Kuhn theorem and the
canonical FOSG-to-EFG bridge.  What is still open is the full bridge
proof that the native round-view carriers and bounded FOSG carriers
realise exactly the same mixed, behavioural, and outcome laws.  The
Lean API exposes this as an explicit argument to the native Kuhn
statement instead of treating it as prose.
...
\paragraph{Solver export.}  The code now has a finite pure
kernel-game export object with player/strategy enumerations and
kernel/utility oracles.  It is deliberately narrower than a
complete solver interchange format, and the PMF-behavioural carrier
is not exported as a finite strategy list.

\paragraph{Decidability.}  The static obligations of
\S\ref{sec:source:wf} — contextual well-formedness, fresh
bindings, reveal completeness, and distribution normalisation —
...
under \Cref{def:stoch-ref}, with the cryptographic soundness
...
\paragraph{Concurrent-strategy realisation.}
\Cref{thm:kuhn} gives the realisation half of the classical
mixed-vs-behavioural correspondence.  In a concurrent-strategy
setting (where a player can commit to a function of \emph{joint}
information across simultaneous events), the corresponding
realisation theorem is more delicate, and the formalism naturally
...
sanity-check theorems or explicit bridge obligations
(payoff-vector agreement, FOSG Kuhn realisation, EFG presentation
