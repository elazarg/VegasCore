...
other, but the compiled game still presents them as simultaneous sealed
choices.
...
makes each one answer a single question: \emph{what may I forget?}
...
\item[\textnormal{R2.} Source\,$\to$\,graph adequacy \quad{\small(adequacy,
  ...
  (\Cref{sec:eg:adequacy}).  This is one-directional --- a faithfulness
  theorem, not a symmetric equivalence between arbitrary programs and
...
\item[\textnormal{R5.} Runtime refinement \quad{\small(order,
  ...
  \emph{Forget:} which administrative runtime details were erased.
\end{description}
...
\node[side] (game)  at (5.6,-1.9){Frontier game\\{\footnotesize beh.\,$\cong$\,mixed-pure}};
...
% R1 confluence: self relation on graph
\draw[rel,<->] (graph.west) .. controls +(-1.7,0.5) and +(-1.7,-0.5) .. (graph.west)
  node[rlab,left=10mm,pos=0.5]{R1~$\eqrel$\\{\scriptsize confluence}};
% execute
\draw[rel] (graph) -- node[rlab,right=1mm]{execute} (mach);
% R4 frontier view + equivalence inside game noted in node
\draw[rel] (mach.east) .. controls +(2.4,0) and +(-1.6,0) .. (game.west)
  node[rlab,pos=0.5,above]{frontier\\view};
\node[rlab] at (5.6,-0.85) {R4~$\eqrel$ \scriptsize(Kuhn)};
% R3 presentation
