...
Each definition is presented as a shaded \emph{Lean box} containing
the actual Lean signature (with implementation noise stripped and
implicit binders elided where they only add clutter), followed by
prose that introduces the mathematical content, draws out
displayed equations where they clarify, and explains why each piece
exists. Defined names appear in monospace and link to their
definition site (e.g.~\DefRef{Vegas.WFProgram}\label{def:Vegas.WFProgram});
cross-references match the names used in the main report. Lean
identifiers that are not Vegas concepts (e.g.\ Mathlib types like
\TT{Finset}, \TT{PMF}) are typeset but not hyperlinked.

We use the macros from the main paper: $\WFP$, $\Outcome$, $\FW$,
$\PMF{\cdot}$, $\bind$, $\Players$, $\view{\cdot}$, $\pubctx{\cdot}$,
$\erasectx{\cdot}$, $\erasepub{\cdot}$, $\Reach$, $\CanReach$.
Additional notation introduced in the supplement is marked locally.
...
\DefRef{kuhn_finite} is a \emph{realisation}
...
compose Kuhn realisation with a separate fixed-point existence theorem.
...
normalisation obligation for the sample, checks the payoff expression
...
It then specialises these theorems to each example (Prisoners,
