...
\paragraph{Independent explicit frontier rounds: \TT{HasIndependentFrontierRounds}.}
\label{def:frontierRounds}
The lift from machine to native round and FOSG views uses the canonical
frontier order $\TT{cfg.frontier.toList}$ as a representative linearization.
The required graph-side condition is
\DefRef{HasIndependentFrontierRounds}\label{def:HasIndependentFrontierRounds}:
it includes
...
independent explicit batches are then witnessed as available primitive
...
event-batch trace and morphism layers.
