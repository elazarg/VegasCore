...
\paragraph{Local explicit frontier steps: \TT{HasLocalFrontierSteps}.}
\label{def:frontierSteps}
...
\DefRef{HasLocalFrontierSteps}\label{def:HasLocalFrontierSteps}:
it includes
...
the primitive event identity: the frontier sampler records
...
For event-graph machines, a \DefRef{PlayerFrontierAction}\label{def:PlayerFrontierAction}
...
For event-graph machines the player-facing menu at a frontier step
is packaged as $\TT{EventGraph.frontierMenu}~G~\TT{cfg}~\TT{who}$: a set
of optional frontier actions where $\TT{none}$ means the player is not
...
\DefRef{eventGraph_frontierMenu_eq_of_observation_eq}\label{def:eventGraph_frontierMenu_eq_of_observation_eq}
...
frontier-action patches; internal nodes use their event-graph kernels.
...
-- round history outcomes\\
def pureRoundHistoryGameFormAt            g\label{def:pureRoundHistoryGameFormAt}\\
def pmfBehavioralRoundHistoryGameFormAt   g\label{def:pmfBehavioralRoundHistoryGameFormAt}\\[3pt]
...
def pureRoundHistoryKernelGameAt          g\\
def pmfBehavioralRoundHistoryKernelGameAt g
\end{leanbox}
...
and round history game forms have canonical utilities and therefore produce
...
\item \emph{Outcome shape}: the payoff vector $\Outcome\,\Players$ versus
  the richer \texttt{syntaxRoundHistoryAt}\,$g$ which retains bounded
  native round histories and determines their realized primitive-event batches.
\end{itemize}
The round history games refine the payoff-vector games by an outcome
...
canonical utility. The round history games set $O$ to
\[
  \mathtt{syntaxRoundHistoryAt}\,g,
\]
...
The relationship between the round history and payoff-vector games is
packaged as a
...
\DefRef{pureRoundHistoryKernelGameAt_eu_eq}\label{def:pureRoundHistoryKernelGameAt_eu_eq}
...
round history and morphism layers.
...
served by the round history presentation.  Its outcome carrier is a
bounded native round history, so its utility depends only on the finite
checkpoint state.  The theorem
\DefRef{pureRoundHistory_mixedNash_exists}\label{def:pureRoundHistory_mixedNash_exists}
therefore obtains a mixed Nash equilibrium for the pure round history
...
def pureRoundHistoryKernelGameExport ...\label{def:pureRoundHistoryKernelGameExport}
\end{leanbox}
...
payoff-vector-game and round history-game expected utilities are
equal
(\DefRef{pure_eu_eq_roundHistory}\label{def:pure_eu_eq_roundHistory}
and
\DefRef{behavioral_eu_eq_roundHistory}\label{def:behavioral_eu_eq_roundHistory}),
and Nash and dominance predicates transport across the projection
(\DefRef{pure_nash_iff_roundHistory}\label{def:pure_nash_iff_roundHistory},
\DefRef{behavioral_nash_iff_roundHistory}\label{def:behavioral_nash_iff_roundHistory},
\DefRef{pure_dominant_iff_roundHistory}\label{def:pure_dominant_iff_roundHistory}).
...
data, and \emph{how} the payoff-vector-game and round history-game
