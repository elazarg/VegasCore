...
its soundness theorems (frontier diamond and explicit-batch execution)
are stated in
...
\item $\TT{patchLegal}$ \emph{(shape)}: a predicate on a single node
  ...
  expected typing. The clear/hidden mode is determined by the node
  semantics, not duplicated in the patch payload. It does \emph{not}
  consult guards or other nodes' results.
...
internal events are $\TT{InternalEvent.node}\,n\,\TT{patch}$ for
non-player frontier nodes together with the realized
$\DefRef{FieldPatch}$, plus an always-unavailable
...
chosen patch. The diamond property and frontier-read invariance lemmas
...
\paragraph{Local explicit frontier rounds: \TT{HasLocalFrontierRounds}.}
\label{def:frontierRounds}
The lift from machine to native round and FOSG views uses an order-free
frontier-realization transition. Canonical frontier lists are retained as
representative primitive-execution witnesses.
The required graph-side condition is
\DefRef{HasLocalFrontierRounds}\label{def:HasLocalFrontierRounds}:
it includes
\DefRef{HasAvailablePlayerActions}\label{def:HasAvailablePlayerActions}
for player-owned frontier nodes and an action-stability condition saying
that executing one frontier node cannot invalidate a legal player action
for a different source-frontier node. It also requires internal-kernel
stability: executing one frontier node cannot change the kernel sampled by a
different source-frontier node. Internal stochastic choices are explicit in
the primitive event identity: the round sampler records
$\TT{InternalEvent.node}\,n\,\TT{patch}$ with the sampled patch. Supported
explicit batches are then witnessed as available primitive
$\TT{runEventsFrom}$ executions. The canonical order is proof/execution
presentation, while the stability fields express the frontier independence
expected from the event-graph abstraction.
...
\bdd PMF M.State\\
\bd eventBatch       : M.State $\to$ JointAction Act $\to$ M.State $\to$ List M.Event\\
...
and the decomposition $\TT{eventBatch}$ that records, from source,
joint move, and realized destination, which list of primitive machine
events the round represents. The
...
round. The explicit event-batch distribution records the concrete
primitive events executed by that canonical linearization, including
sampled patches for internal nodes, and its support theorem relates
each recorded batch back to primitive $\TT{runEventsFrom}$ execution.
...
-- realization trace outcomes\\
def pureRealizationTraceGameFormAt            g\label{def:pureRealizationTraceGameFormAt}\\
def pmfBehavioralRealizationTraceGameFormAt   g\label{def:pmfBehavioralRealizationTraceGameFormAt}\\[3pt]
...
def pureRealizationTraceKernelGameAt          g\\
def pmfBehavioralRealizationTraceKernelGameAt g
\end{leanbox}
...
and realization trace game forms have canonical utilities and therefore produce
...
\item \emph{Outcome shape}: the payoff vector $\Outcome\,\Players$ versus
  the richer \texttt{syntaxRealizationTraceAt}\,$g$ which retains bounded
  native round histories and their realized primitive-event batches.
\end{itemize}
The realization trace games refine the payoff-vector games by an outcome
...
canonical utility. The realization trace games set $O$ to
\[
  \mathtt{syntaxRealizationTraceAt}\,g,
\]
the bounded native round-history type, with utility read from the public
outcome of the last checkpoint state.

\paragraph{Refinement layer.}
The relationship between the realization trace and payoff-vector games is
packaged as a
...
\DefRef{pureRealizationTraceKernelGameAt_eu_eq}\label{def:pureRealizationTraceKernelGameAt_eu_eq}
...
terminal. Strategic equivalence is then proved through the explicit
realization trace and morphism layers.
...
served by the realization trace presentation.  Its outcome carrier is a
bounded native round history, so its utility depends only on the finite
checkpoint state.  The theorem
\DefRef{pureRealizationTrace_mixedNash_exists}\label{def:pureRealizationTrace_mixedNash_exists}
therefore obtains a mixed Nash equilibrium for the pure realization trace
...
information condition is \TT{LegalObservable}\label{def:LegalObservable} for the bounded FOSG,
proved for checked program event graphs from the visibility and graph-shape
...
def pureRealizationTraceKernelGameExport ...\label{def:pureRealizationTraceKernelGameExport}
\end{leanbox}
...
\item \DefRef{PureSpecEventBatchLaw}~$g$ is a per-pure-profile specification
  event-batch law plus a proof that it reproduces the canonical explicit
  event-batch trace distribution induced by the native round view.
...
\item \DefRef{toBackendEventBatchTraceMorphism} and its behavioral
  ...
  \[
    \text{backend game} \;\longrightarrow\; \text{public spec game},
  \]
...
implementation into the canonical Vegas public game. The
...
event-batch traces — the code asks for a trace-level soundness proof
rather than accepting outcome-level agreement. For the identity refinement,
...
The composed ``payoff-vector game to backend'' statement is direct here:
the backend morphism already targets the public payoff-vector game. The code
therefore uses the same morphism API without repeating expected-utility
...
payoff-vector-game and realization trace-game expected utilities are
equal
(\DefRef{pure_eu_eq_realizationTrace}\label{def:pure_eu_eq_realizationTrace}
and
\DefRef{behavioral_eu_eq_realizationTrace}\label{def:behavioral_eu_eq_realizationTrace}),
and Nash and dominance predicates transport across the projection
(\DefRef{pure_nash_iff_realizationTrace}\label{def:pure_nash_iff_realizationTrace},
\DefRef{behavioral_nash_iff_realizationTrace}\label{def:behavioral_nash_iff_realizationTrace},
\DefRef{pure_dominant_iff_realizationTrace}\label{def:pure_dominant_iff_realizationTrace}).
...
data, and \emph{how} the payoff-vector-game and realization trace-game
