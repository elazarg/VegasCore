...
\item $\TT{sem}$, \DefRef{prereqs}, \DefRef{sliceLegal},
...
$\TT{PlayerAction}\,G\,p$, pairs $(n,\,\TT{slice})$ with
$n : G.\TT{Node}$ and $\TT{slice} : G.\DefRef{WriteSlice}$;
...
with a legal slice. $\TT{stepPlay}$ stutters on unavailable actions
(returns $\mathrm{PMF.pure}\,\mathrm{cfg}$) and otherwise commits the
chosen slice. The diamond property and frontier-read stability lemmas
...
is a total assignment of candidate write slices to graph nodes; the
...
frontier node they own. Irrelevant coordinates of the total slice
...
round-action slices; internal nodes use their event-graph kernels.
...
\DefRef{rowDefectSlice}\label{def:rowDefectSlice},
\DefRef{rowDefectAction}\label{def:rowDefectAction} — exercising
\DefRef{sliceLegal} and \DefRef{actionLegal} directly. This is the
