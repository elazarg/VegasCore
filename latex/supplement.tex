...
\TT{Backend/}
  & Machine-to-machine refinement (weak and stochastic) and
    blocked-trace kernel-game transport from a backend
    implementation. \\
\TT{Theorems/}
...
\bdd PMF M.State\\
\bd eventBlock       : M.State $\to$ JointAction Act $\to$ List M.Event\\
...
and the decomposition $\TT{eventBlock}$ that records which list of
...
$\TT{eventBlock}$ is replaced by a per-step scalar reward
...
served by the blocked-trace presentation.  Its outcome carrier is
...
\DefRef{pureBlockedTrace_mixedNash_exists}\label{def:pureBlockedTrace_mixedNash_exists}
therefore obtains a mixed Nash equilibrium for the pure blocked-trace
...
\item \DefRef{toBackendBlockedTraceMorphism} and its behavioral
...
event blocks, block traces, compatible block laws, and blocked-trace
...
payoff-vector-game and blocked-trace-game expected utilities are
equal
(\DefRef{pure_eu_eq_blockedTrace}\label{def:pure_eu_eq_blockedTrace}
and
\DefRef{behavioral_eu_eq_blockedTrace}\label{def:behavioral_eu_eq_blockedTrace}),
and Nash and dominance predicates transport across the projection
(\DefRef{pure_nash_iff_blockedTrace}\label{def:pure_nash_iff_blockedTrace},
\DefRef{behavioral_nash_iff_blockedTrace}\label{def:behavioral_nash_iff_blockedTrace},
\DefRef{pure_dominant_iff_blockedTrace}\label{def:pure_dominant_iff_blockedTrace}).
...
data, and \emph{how} the payoff-vector-game and blocked-trace-game
