...
structure PlayerStrategy (who : Player) where
  commit : {Γ : CtxSimple} → {b : BaseTy} → (x : VarId) →
    (acts : List (Val b)) →
    (R : Expr ((x, .pub b) :: flattenCtx (viewCtx who Γ)) .bool) →
    CommitKernelSimple who Γ b
  normalized : {Γ : CtxSimple} → {b : BaseTy} → (x : VarId) →
    (acts : List (Val b)) →
    (R : Expr ((x, .pub b) :: flattenCtx (viewCtx who Γ)) .bool) →
    (view : EnvSimple (viewCtx who Γ)) →
    FDist.totalWeight (commit x acts R view) = 1

/-- Assemble per-player strategy components into a Vegas `ProfileSimple`. -/
def toProfile (σ : ∀ who, PlayerStrategy who) : ProfileSimple where
  commit := fun who x acts R view => (σ who).commit x acts R view

/-- Bundled player strategies are normalized on every Vegas program. -/
theorem toProfile_normalizedOn (σ : ∀ who, PlayerStrategy who) (p : VegasSimple Γ) :
...
noncomputable def toKernelGame (p : VegasSimple Γ) (env : EnvSimple Γ)
    (hd : NormalizedDists p) : GameTheory.KernelGame Player where
  Strategy := PlayerStrategy
  Outcome := Outcome
  utility := fun o i => (payoffKitSimple.payoff o i : ℝ)
  outcomeKernel := fun σ =>
...
@[simp] theorem toKernelGame_outcomeKernel
    (p : VegasSimple Γ) (env : EnvSimple Γ) (hd : NormalizedDists p)
    (σ : ∀ who, PlayerStrategy who) :
...
theorem toKernelGame_eu (p : VegasSimple Γ) (env : EnvSimple Γ) (hd : NormalizedDists p)
  ...
  simpa [GameTheory.KernelGame.eu, toKernelGame, hnorm, NNRat.toNNReal_coe_real] using
    (FDist.expect_toPMF_eq_sum
      ...
      (h := hnorm)
      (f := fun o => (payoffKitSimple.payoff o who : ℝ)))
