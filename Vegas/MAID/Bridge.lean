...
private noncomputable abbrev bridgeState
    (B : MAIDBackend Player L)
    {Γ : VCtx Player L}
    (p : VegasCore Player L Γ)
...
private noncomputable abbrev bridgeStruct
    (B : MAIDBackend Player L)
    {Γ : VCtx Player L}
    (p : VegasCore Player L Γ)
    (hl : Legal p) (hd : NormalizedDists p)
    (ρ : RawNodeEnv L → VEnv L Γ)
    (st₀ : MAIDCompileState Player L B) :=
  (bridgeState B p hl hd ρ st₀).toStruct
...
private abbrev BridgeTailIH
  ...
  ∀ (hl : Legal p) (hd : NormalizedDists p) (hfresh : FreshBindings p)
    ...
    BridgeInv B p hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup

private theorem bridgeInv_ret_fold
    (B : MAIDBackend Player L)
    {Γ : VCtx Player L}
      ...
      PMF.map (fun a =>
        ...
        (((List.finRange (bridgeState B (.ret u) hl hd ρ st₀).nextId).drop st₀.nextId)
          |>.foldl
            ...
            (PMF.pure a₀)) =
      nativeOutcomeDistPMFV B (.ret u) hd
...
private theorem bridgeInv_ret_behavioral
    (B : MAIDBackend Player L)
    {Γ : VCtx Player L}
    (u : List (Player × L.Expr (erasePubVCtx Γ) L.int))
    (hl : Legal (.ret u)) (hd : NormalizedDists (.ret u))
...
private theorem bridgeInv_ret
  ...
  exact ⟨bridgeInv_ret_fold B u hl hd ρ st₀,
    bridgeInv_ret_behavioral B u hl hd ρ st₀⟩

private theorem bridgeInv_letExpr_fold
    (B : MAIDBackend Player L)
      ...
      PMF.map (fun a =>
        ...
        (((List.finRange (MAIDCompileState.ofProg B (.letExpr x e k) hl hd ρ st₀).nextId).drop
          st₀.nextId) |>.foldl
            ...
            (PMF.pure a₀)) =
      nativeOutcomeDistPMFV B (.letExpr x e k) hd
...
private theorem bridgeInv_letExpr_behavioral
    (B : MAIDBackend Player L)
    {Γ' : VCtx Player L} {x : VarId} {b : L.Ty}
    (e : L.Expr (erasePubVCtx Γ') b)
    (k : VegasCore Player L ((x, .pub b) :: Γ'))
...
private theorem bridgeInv_letExpr
  ...
  exact ⟨bridgeInv_letExpr_fold B e k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup,
    bridgeInv_letExpr_behavioral B e k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup⟩

private theorem bridgeInv_reveal_fold
    (B : MAIDBackend Player L)
...
private theorem bridgeInv_reveal_behavioral
    (B : MAIDBackend Player L)
    {Γ' : VCtx Player L} {y x : VarId} {who : Player} {b : L.Ty}
    (hx : VHasVar Γ' x (.hidden who b))
    (k : VegasCore Player L ((y, .pub b) :: Γ'))
...
private theorem bridgeInv_reveal
  ...
  exact ⟨bridgeInv_reveal_fold B hx k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup,
    bridgeInv_reveal_behavioral B hx k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup⟩

private theorem bridgeInv_sample_fold
    (B : MAIDBackend Player L)
    {Γ' : VCtx Player L} {x : VarId} {b : L.Ty}
    (D' : L.DistExpr (erasePubVCtx Γ') b)
    (k : VegasCore Player L ((x, .pub b) :: Γ'))
    (ih : BridgeTailIH B k)
    (hl : Legal (.sample x D' k)) (hd : NormalizedDists (.sample x D' k))
    (hfresh : FreshBindings (.sample x D' k))
    (ρ : RawNodeEnv L → VEnv L Γ')
    (st₀ : MAIDCompileState Player L B)
    (hvars : st₀.VarsSubCtx Γ')
    (hρ_deps : ∀ j, j ∉ (st₀.ctxDeps Γ' : Finset Nat) → InsensitiveTo ρ j)
    ...
    ∀ (_hρ_readers : ViewDeterminesRaw st₀ Γ' ρ)
      ...
      PMF.map (fun a =>
          extractOutcomeAux B (.sample x D' k) ρ st₀.nextId
            (rawOfTAssign (MAIDCompileState.ofProg B (.sample x D' k) hl hd ρ st₀) a))
        (((List.finRange
          ...
          st₀.nextId) |>.foldl
            ...
            (PMF.pure a₀)) =
      nativeOutcomeDistPMFV B (.sample x D' k) hd
  ...
  let cpdFDist : RawNodeEnv L → FDist (L.Val b) := fun raw =>
    L.evalDist D' (VEnv.eraseSampleEnv (ρ raw))
  let cpdNorm : ∀ raw, FDist.totalWeight (cpdFDist raw) = 1 := fun raw => hd.1 _
  let nd : CompiledNode Player L B := .chance b (st₀.ctxDeps Γ') cpdFDist cpdNorm
  ...
  let ρ' : RawNodeEnv L → VEnv L ((x, .pub b) :: Γ') :=
    fun raw => VEnv.cons (τ := .pub b) (MAIDCompileState.readVal (B := B) raw b st₀.nextId) (ρ raw)
  have hvars₁ : st₁.VarsSubCtx ((x, .pub b) :: Γ') := by
    simpa [st₁, stNode, nd] using
      st₀.VarsSubCtx_addNode_addVar_singleton_step hvars nd hndeps x (.pub b) hxΓ
  have hctx₁ : st₁.ctxDeps ((x, .pub b) :: Γ') = {st₀.nextId} ∪ st₀.ctxDeps Γ' := by
    simpa [st₁, stNode, nd] using
      st₀.ctxDeps_addNode_addVar_singleton_cons_eq_of_fresh nd hndeps x (.pub b) hxΓ hxvars
  have hρ'_deps : ∀ j, j ∉ st₁.ctxDeps ((x, .pub b) :: Γ') → InsensitiveTo ρ' j := by
    intro j hj raw tv
    have hjid : j ≠ st₀.nextId := by intro hEq; apply hj; simp [hctx₁, hEq]
    have hj' : j ∉ st₀.ctxDeps Γ' := by intro hmem; apply hj; simp [hctx₁, hmem]
    have hρj := hρ_deps j hj' raw tv
    exact VEnv.cons_ext (readVal_extend_ne raw j st₀.nextId tv b hjid.symm) hρj
  have hρ'_var : EnvRespectsLookupDeps st₁ ρ' := by
  ...
  change PMF.map (fun a => extractOutcomeAux B (.sample x D' k)
    ρ st₀.nextId (rawOfTAssign st a))
    (List.foldl (evalStep st.toStruct st.toSem pol) (PMF.pure a₀)
      ((List.finRange st.nextId).drop st₀.nextId)) =
    nativeOutcomeDistPMFV B (.sample x D' k) hd
      (reflectPolicyAuxV B (.sample x D' k) hl hd ρ st₀ pol) ρ
  ...
  have hρ'_readers : ViewDeterminesRaw st₁ ((x, .pub b) :: Γ') ρ' := by
    ...
    have hVD : st₁.viewDeps who ((x, .pub b) :: Γ') = insert st₀.nextId (st₀.viewDeps who Γ') := by
      unfold MAIDCompileState.viewDeps
      simp only [viewVCtx, canSee, ite_true, List.map_cons, MAIDCompileState.depsOfVars]
      rw [stNode.lookupDeps_addVar_eq_self_of_fresh x (.pub b) {st₀.nextId}
          (by
            intro d hd_
          ...
          (by simpa [stNode, MAIDCompileState.addNode] using hxvars),
        stNode.depsOfVars_addVar_eq_of_not_mem x (.pub b) _ _ _ hx_not_view,
    ...
    have hraw_id_eq : raw₁ st₀.nextId = raw₂ st₀.nextId := by
      have hhead := projectViewEnv_cons_head_eq
        (List.nodup_cons.mpr ⟨hxΓ, hnodup⟩) (by simp [canSee]) hview
    ...
    · apply hρ_readers who raw₁ raw₂
      ...
      · exact hview_old
      · exact hold
  have hρ'_readval : EnvReadValAtDeps st₁ ((x, .pub b) :: Γ') ρ' := by
    ...
    | there hy' =>
      ...
      exact ⟨j, by rw [hst₁_id]; omega, by rwa [hld_st₁_st₀],
        ...
        fun raw => by simpa [ρ', VEnv.get, VEnv.cons_get_there] using hget raw⟩
  have hinner : ∀ v, PMF.map (fun a => extractOutcomeAux B (.sample x D' k) ρ
        st₀.nextId (rawOfTAssign st a))
    ...
    · rename_i hk τ₁ deps₁ cpd₁ norm₁ hdesc₁
      ...
      let F : CompiledNode.valType nd → PMF (Outcome Player) :=
        fun w => nativeOutcomeDistPMFV B k hd.2
          ...
          (st₀.nextId + 1) ((rawOfTAssign st a₀).extend st₀.nextId ⟨_, w⟩)
      ...
      congr 1
      exact congrArg (fun env => FDist.toPMF (L.evalDist D' (VEnv.eraseSampleEnv env))
        (hd.1 env)) hρeq
    · rename_i hdesc₁
...
private theorem bridgeInv_sample_behavioral
    (B : MAIDBackend Player L)
    {Γ' : VCtx Player L} {x : VarId} {b : L.Ty}
    (D' : L.DistExpr (erasePubVCtx Γ') b)
    (k : VegasCore Player L ((x, .pub b) :: Γ'))
  ...
  let nd : CompiledNode Player L B := .chance b (st₀.ctxDeps Γ')
    (fun raw => L.evalDist D' (VEnv.eraseSampleEnv (ρ raw))) (fun raw => hd.1 _)
  have hndeps : ∀ d ∈ nd.parents ∪ nd.obsParents, d < st₀.nextId := by
  ...
  exact (ih hl hd.2 hfresh.2 _ _
    (st₀.VarsSubCtx_addNode_addVar_singleton_step hvars _ _ x (.pub b) hxΓ)
    (fun j hj raw tv => by
    ...
    (realizedRawEnv_extend_addNode_addVar st₀ ρ env
      ⟨raw₀, hraw₀, hraw_typed, hraw_hi⟩ hρ_deps nd hndeps
      (x := x) (τ := .pub b) v
      ⟨v, rfl⟩)

private theorem bridgeInv_sample
    (B : MAIDBackend Player L)
    {Γ' : VCtx Player L} {x : VarId} {b : L.Ty}
    (D' : L.DistExpr (erasePubVCtx Γ') b)
    (k : VegasCore Player L ((x, .pub b) :: Γ'))
    ...
    (hρ_var : EnvRespectsLookupDeps st₀ ρ)
    (hnodup : (Γ'.map Prod.fst).Nodup) :
    BridgeInv B (.sample x D' k) hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup := by
  exact ⟨bridgeInv_sample_fold B D' k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup,
    bridgeInv_sample_behavioral B D' k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup⟩

private theorem bridgeInv_commit_fold
    (B : MAIDBackend Player L)
    {Γ' : VCtx Player L} {x : VarId} {who_commit : Player} {b : L.Ty}
    (R : L.Expr ((x, b) :: eraseVCtx Γ') L.bool)
    (k : VegasCore Player L ((x, .hidden who_commit b) :: Γ'))
    (ih : BridgeTailIH B k)
    (hl : Legal (.commit x who_commit R k)) (hd : NormalizedDists (.commit x who_commit R k))
    (hfresh : FreshBindings (.commit x who_commit R k))
    (ρ : RawNodeEnv L → VEnv L Γ')
    (st₀ : MAIDCompileState Player L B)
    (hvars : st₀.VarsSubCtx Γ')
    (hρ_deps : ∀ j, j ∉ (st₀.ctxDeps Γ' : Finset Nat) → InsensitiveTo ρ j)
  ...
  let ρ' : RawNodeEnv L → VEnv L ((x, .hidden who_commit b) :: Γ') :=
...
private theorem commit_cfg_view_exists_of_realizedRawEnv
    (B : MAIDBackend Player L)
    {Γ' : VCtx Player L} {x : VarId} {who_commit : Player} {b : L.Ty}
    (k : VegasCore Player L ((x, .hidden who_commit b) :: Γ'))
    (hlk : Legal k) (hdk : NormalizedDists k)
    (ρ : RawNodeEnv L → VEnv L Γ')
    (st₀ : MAIDCompileState Player L B)
    (hρ_var : EnvRespectsLookupDeps st₀ ρ)
    (env : VEnv L Γ')
    ...
    (hnd : nd = .decision b who_commit (allValues B b) (allValues_ne_nil B b)
    ...
    let ρ' : RawNodeEnv L → VEnv L ((x, .hidden who_commit b) :: Γ') :=
      fun raw => VEnv.cons (τ := .hidden who_commit b)
        (MAIDCompileState.readVal (B := B) raw b st₀.nextId) (ρ raw)
    let st := MAIDCompileState.ofProg B k hlk hdk ρ' st₁
  ...
  let cfg : MAID.Cfg (fp := B.fintypePlayer) st.toStruct (st.toStruct.obsParents nd0) := by
    ...
    have hdesc_i : st.descAt i.1 = st₀.descAt ⟨i.1.val, hi_old⟩ := by
      have hprog_old := MAIDCompileState.ofProg_descAt_old B k hlk hdk ρ' st₁ i.1.val
        (by
    ...
    · by_cases hmem : i ∈ st₀.viewDeps who_commit Γ'
      · have hi_obs : (⟨i, hi⟩ : Fin st.nextId) ∈ st.toStruct.obsParents nd0 :=
        ...
        have hdesc_i : st.descAt ⟨i, hi⟩ = st₀.descAt ⟨i, hi_old⟩ := by
          have hprog_old := MAIDCompileState.ofProg_descAt_old B k hlk hdk ρ' st₁ i
            (by
              have hnext : st₁.nextId = st₀.nextId + 1 := by
                simp [st₁, stNode, MAIDCompileState.addVar, MAIDCompileState.addNode]
              omega)
  ...
  have hraw_cfg_old : st.rawEnvOfCfg cfg =
    ...
    · have hi₁ : i < st₁.nextId := by
      ...
      have hi : i < st.nextId := Nat.lt_of_lt_of_le hi₁
        (MAIDCompileState.ofProg_nextId_le B k hlk hdk ρ' st₁)
      simp [hi, hi₀]
...
private theorem bridgeInv_commit_behavioral
    (B : MAIDBackend Player L)
    {Γ' : VCtx Player L} {x : VarId} {who_commit : Player} {b : L.Ty}
    (R : L.Expr ((x, b) :: eraseVCtx Γ') L.bool)
    (k : VegasCore Player L ((x, .hidden who_commit b) :: Γ'))
  ...
  · intro v
    ...
    exact (ih hl.2 hd hfresh.2 ρ' st₁
      ...
      (realizedRawEnv_extend_addNode_addVar st₀ ρ env
            ⟨raw₀, hraw₀, hraw_typed, hraw_hi⟩ hρ_deps nd hndeps
            (x := x) (τ := .hidden who_commit b) v
            ⟨v, Option.mem_def.mp rfl⟩)

private theorem bridgeInv_commit
    (B : MAIDBackend Player L)
    {Γ' : VCtx Player L} {x : VarId} {who_commit : Player} {b : L.Ty}
    (R : L.Expr ((x, b) :: eraseVCtx Γ') L.bool)
    (k : VegasCore Player L ((x, .hidden who_commit b) :: Γ'))
    ...
    (hρ_var : EnvRespectsLookupDeps st₀ ρ)
    (hnodup : (Γ'.map Prod.fst).Nodup) :
    BridgeInv B (.commit x who_commit R k) hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup := by
  exact ⟨bridgeInv_commit_fold B R k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup,
    bridgeInv_commit_behavioral B R k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup⟩
...
private theorem bridgeInv
  ...
  | sample x D' k ih =>
      exact bridgeInv_sample B D' k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup
  | reveal y who x hx k ih =>
      exact bridgeInv_reveal B hx k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup
  | commit x who_commit R k ih =>
      exact bridgeInv_commit B R k ih hl hd hfresh ρ st₀ hvars hρ_deps hρ_var hnodup

private theorem bridgeInv_empty_const
    (B : MAIDBackend Player L)
    {Γ : VCtx Player L}
    (p : VegasCore Player L Γ)
    (env : VEnv L Γ)
    ...
    (hfresh : FreshBindings p)
    (hnodup : (Γ.map Prod.fst).Nodup) :
    BridgeInv B p hl hd hfresh (fun _ => env) .empty
      (fun _ h => by simp [MAIDCompileState.empty] at h)
      (fun j _ => by intro raw tv; rfl)
      (envRespectsLookupDeps_const (B := B) .empty env)
      hnodup := by
  exact bridgeInv B p hl hd hfresh (fun _ => env) .empty
    (fun _ h => by simp [MAIDCompileState.empty] at h)
    (fun j _ => by intro raw tv; rfl)
    (envRespectsLookupDeps_const (B := B) .empty env)
    hnodup

private theorem viewDeterminesRaw_empty_const
    (B : MAIDBackend Player L)
    {Γ : VCtx Player L}
    (env : VEnv L Γ) :
    ViewDeterminesRaw (B := B) .empty Γ (fun _ => env) := by
  intro who raw₁ raw₂ _ _ _ _ i hi
  simp only [MAIDCompileState.viewDeps, MAIDCompileState.empty] at hi
  exact absurd hi (by
    have := MAIDCompileState.depsOfVars_lt (st := (MAIDCompileState.empty (B := B) (L := L)))
      ((viewVCtx who Γ).map Prod.fst) i hi
    simp [MAIDCompileState.empty] at this)

private theorem envReadValAtDeps_empty_const
    (B : MAIDBackend Player L)
    {Γ : VCtx Player L}
    (env : VEnv L Γ) :
    EnvReadValAtDeps (B := B) .empty Γ (fun _ => env) := by
  intro x who bt hx hne
  exact absurd hne (by
    simp [MAIDCompileState.empty, MAIDCompileState.lookupDeps,
      MAIDCompileState.lookupDepsAux, Finset.not_nonempty_empty])

private theorem rawWitness_empty_const
    (B : MAIDBackend Player L)
    {Γ : VCtx Player L}
    (env : VEnv L Γ) :
    RealizedRawEnv (MAIDCompileState.empty (B := B) (L := L)) (fun _ => env) env := by
  refine ⟨fun _ => (none : Option (RawTaggedVal L)), rfl, ?_, ?_⟩
  · intro j hj
    exact absurd hj (by simp [MAIDCompileState.empty])
  · intro j hj
    rfl
...
theorem vegasMAID_reverse_bridge
  ...
  have hinv := bridgeInv_empty_const B p env hl hd hfresh hnodup
  have hbridge := hinv.fold_eq
    (viewDeterminesRaw_empty_const B env)
    (envReadValAtDeps_empty_const B env)
    pol (defaultAssign S)
  rw [show (MAIDCompileState.empty (B := B) (L := L)).nextId = 0 from rfl,
