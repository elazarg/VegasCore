...
* `Vegas.Protocol.EventGraph` is the finite dependency/visibility language for
...
* `Vegas.Protocol.EventGraphMachine` interprets an `EventGraph.Configuration` as
...
* `Vegas.Protocol.ProgramEventGraph` elaborates checked Vegas programs to an
  `EventGraph`, defines the canonical `eventGraphMachine`, and proves the
...
* `Vegas.Protocol.FOSG` derives checkpoint FOSG views directly from `Machine`
  ...
  alphabet.
* `Vegas.Protocol.Strategic` packages the finite event-graph FOSG view as
