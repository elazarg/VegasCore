...
theorem outcomeKernel_eq_of_traceOutcome_eq
    ...
    (h :
      PMF.map (syntaxGraphTraceOutcome g) left =
        PMF.map (syntaxGraphTraceOutcome g) right) :
    PMF.map (syntaxGraphTraceOutcome g) left =
      PMF.map (syntaxGraphTraceOutcome g) right :=
  h
...
theorem terminalOutcome_public_only
  ...
  rcases syntaxGraphOutcome_eq_evalPayoffs_of_terminal g hleftTerminal with
    ⟨leftEnv, hleftEnv, _hleftOutcome⟩
  rcases syntaxGraphOutcome_eq_evalPayoffs_of_terminal g hrightTerminal with
...
theorem declared_payoff_rule_unique
    ...
    (hleft :
      ProgramField.finalEnv? g.prog (syntaxGraphConfigValue? g cfg) =
        some leftEnv)
    (hright :
      ProgramField.finalEnv? g.prog (syntaxGraphConfigValue? g cfg) =
        some rightEnv) :
