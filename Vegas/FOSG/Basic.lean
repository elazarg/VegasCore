...
The cursor state must remember not only the current subprogram but also where
that subprogram sits inside the original Vegas program. This lets syntax-facing
adapters identify the current continuation without putting strategy data into
the machine state.
-/
