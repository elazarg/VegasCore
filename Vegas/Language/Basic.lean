...
import Vegas.Language.Obligations
...
`VegasLang` is a small surface syntax over the concrete `simpleExpr` language
that lowers to the generic `SurfaceCore simpleExpr`. This concrete
...
inductive VegasLang (P : Type) [DecidableEq P] :
  ...
  `SurfaceCore` contexts are SSA-style: the revealed public alias must be fresh
