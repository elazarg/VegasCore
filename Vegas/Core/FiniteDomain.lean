...
This is supporting evidence for the load-bearing `Vegas.VegasCore` syntax in
`Vegas.Core.Basic`; it is deliberately kept out of that module so the core
program syntax can be audited on its own. Finite enumeration of concrete
environments builds on this evidence in `Vegas.Core.Finite`.
-/
