...
import Vegas.Core.ToEventGraph.Compiler
...
`Vegas.Core.Theorems.Claims`: where that file curates the main *theorems*,
...
generic over it. (`Vegas.Base.Basic`)
-/
...
well-scoped by construction. (`Vegas.Base.Basic`)
-/
...
about" means. (`Vegas.Core.Program`)
-/
...
`Vegas.Core.ToEventGraph.Compiler`)
-/
...
rest — are stated in `Vegas.Core.Theorems.Claims`.

## Trusted base

Auditing reduces to reading the definitions above and the claim *statements* in
`Vegas.Core.Theorems.Claims`; the proofs are checked by Lean's kernel. The only
