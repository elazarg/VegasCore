...
Property boundary: this file proves a scheduler invariant: every bounded
runtime trace has no extracted plaintext before both sealed moves are locked.
The invariant is by construction, not cryptographic hiding or a general
no-profitable-front-running theorem.
...
appear only after both moves are locked in the sealed source state.

The checked claim is protocol faithfulness plus no plaintext in the runtime
buffers before the second lock completes. It is not a cryptographic hiding
theorem or a general theorem that all front-running deviations are unprofitable.
-/
...
theorem commitRevealSourceBatch_available
    (profile : ∀ player : Player, CommitRevealStrategy player)
    (state : lockedMPMachine.State) :
    lockedMPMachine.AvailableBatchFrom state
      (commitRevealSourceBatch profile state) := by
  rcases state with
    ⟨aliceMove, bobMove, aliceRevealed, bobRevealed⟩
          ...
          · let move := commitRevealActionProfile profile Player.alice
            ...
            exact Machine.AvailableBatchFrom.singleton
              (by
                ...
                trivial)
          all_goals exact Machine.AvailableBatchFrom.nil _
      | some bobMove =>
...
theorem commitRevealMessageBatch_available
    ...
    commitRevealMessageMachine.AvailableBatchFrom state
      (commitRevealMessageBatch profile state) := by
          ...
          · let move := commitRevealActionProfile profile Player.alice
            ...
            let afterSend : commitRevealMessageMachine.State :=
              { source := src.source,
                ...
                delivered := delivered }
            change commitRevealMessageMachine.AvailableBatchFrom src
              [.play Player.alice (.send .commit),
                .play Player.alice (.spec (.lock move))]
                ...
                (by
                  ...
                  · right
                    ...
                    simp [lockedMPMachine, LockedMPState.setMove, afterSend,
                      src] at htarget
                    subst target
                    simp [lockedMPMachine, LockedMPState.outcome?]
                      at hterminal)
          all_goals
            exact Machine.AvailableBatchFrom.nil _
      | some bobMove =>
  ...
  | some aliceMove =>
      ...
      | none =>
          ...
          · let move := commitRevealActionProfile profile Player.bob
            ...
            change commitRevealMessageMachine.AvailableBatchFrom src
              [.play Player.bob (.send .commit),
                .play Player.bob (.spec (.lock move))]
            refine Machine.AvailableBatchFrom.cons ?_ ?_
            · change Machine.MessageInFlightAction.send
                CommitRevealMessage.commit ∈
                ...
                (by
                  ...
                  · right
                    ...
                    simp [lockedMPMachine, LockedMPState.outcome?]
                      at hterminal)
          · exact
              Machine.AvailableBatchFrom.nil _
          · let sent := revealMessage Player.bob bobMove
            let src : commitRevealMessageMachine.State :=
              { source :=
                ...
                delivered := delivered }
            have hsourceBatch :
                lockedMPMachine.AvailableBatchFrom src.source
            ...
            simpa [commitRevealMessageMachine,
              Machine.messageInFlight.liftEvent, src, sent] using
              Machine.messageInFlight.deliverAllThenSendDeliverLiftAvailableBatchFrom
                lockedMPMachine CommitRevealMessageOf pending delivered
                Player.bob (.reveal bobMove)
                (by
                  simpa [commitRevealMessageMachine,
...
def commitRevealBuffersOpaqueBeforeBothLocked
    (state : commitRevealMessageMachine.State) : Prop :=
  state.source.aliceMove = none ∨ state.source.bobMove = none →
    commitRevealPlaintextPolicy.plaintexts
      (state.pending ++ state.delivered) = []

/-- Every bounded trace generated by the commit/reveal scheduler has no
extracted plaintext in the public buffers until both sealed moves are locked.
Once both moves are locked, reveal messages may appear and this predicate stops
claiming opacity. -/
theorem commitRevealMessageLaw_buffers_opaque_before_both_locked
    (profile : ∀ player : Player, CommitRevealStrategy player)
    (horizon : Nat) (trace : commitRevealMessageMachine.EventBatchTrace)
    (htrace : trace ∈ (commitRevealMessageMachine.eventBatchTraceDist
      (commitRevealMessageLaw profile) horizon).support) :
    commitRevealBuffersOpaqueBeforeBothLocked trace.2 := by
  cases horizon with
  | zero =>
      simp [Machine.eventBatchTraceDist, Machine.eventBatchTraceDistFrom,
        commitRevealMessageMachine, Machine.messageInFlight] at htrace
      subst trace
  ...
  | succ h1 =>
      ...
      | zero =>
          simp [Machine.eventBatchTraceDist, Machine.eventBatchTraceDistFrom,
            ...
            LockedMPState.outcome?, commitRevealActionProfile] at htrace
          subst trace
          simp [commitRevealBuffersOpaqueBeforeBothLocked,
            Machine.PlaintextPolicy.plaintexts, commitRevealPlaintextPolicy]
      | succ h2 =>
          cases h2 with
          | zero =>
              simp [Machine.eventBatchTraceDist, Machine.eventBatchTraceDistFrom,
                commitRevealMessageLaw, commitRevealMessageBatch,
                commitRevealMessageMachine, Machine.messageInFlight,
                lockedMPMachine, Machine.runEventBatchesFrom,
              ...
              subst trace
              simp [commitRevealBuffersOpaqueBeforeBothLocked,
                Machine.PlaintextPolicy.plaintexts,
                commitRevealPlaintextPolicy, aliceCommitView, commitMessage]
          | succ h3 =>
              cases h3 with
              | zero =>
...
opaque commits in the visible buffers, no extracted plaintext, and both sealed
moves locked. -/
