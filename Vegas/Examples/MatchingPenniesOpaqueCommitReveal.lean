...
runtime trace has no extracted plaintext before both hidden moves are locked.
...
extracted plaintext in the public buffers until both hidden moves are locked.
