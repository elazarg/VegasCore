...
/-- Endpoint-parameterized instruction induction.  The one-step premise may
inspect each supported residual run to the fixed endpoints; this is what lets
public future outputs couple live chance and nonfocal private draws without
turning them into shared kernels. -/
theorem runPolicies_replay
    {runtime : GraphRuntime Player L Δ} {whole : Graph Player L Γ₀ Δ}
    ...
    (initial : NativeReplayInvariant runtime whole focal left right)
    (leftSupported : leftFinal ∈
    ...
    (step : ∀ (invocation : @MessageApplication.Invocation Player) rest
      ...
      leftNext ∈ (runtime.application.invoke players environment leftCurrent invocation).support →
      rightNext ∈
        (runtime.application.invoke players environment rightCurrent invocation).support →
      leftFinal ∈
        (runtime.application.runPolicies players environment rest leftNext).support →
      rightFinal ∈
    ...
    NativeReplayInvariant runtime whole focal leftFinal rightFinal := by
  induction schedule generalizing left right with
  | nil =>
      simp only [MessageApplication.runPolicies, FinDist.mem_support_pure] at leftSupported
      simp only [MessageApplication.runPolicies, FinDist.mem_support_pure] at rightSupported
      subst leftFinal
      subst rightFinal
      exact initial
  | cons invocation rest ih =>
      simp only [MessageApplication.runPolicies, FinDist.support_bind, Set.mem_iUnion]
        at leftSupported rightSupported
      obtain ⟨leftNext, leftInvoke, leftTail⟩ := leftSupported
      obtain ⟨rightNext, rightInvoke, rightTail⟩ := rightSupported
      have nextInvariant := step invocation rest left right leftNext rightNext initial
        leftInvoke rightInvoke leftTail rightTail
      exact ih nextInvariant leftTail rightTail

/-- Endpoint-conditioned replay with the concrete processed prefix exposed at
each step.  Unlike `runPolicies_replay`, this form carries actual reachability
from both initialized executions, so resolve-endpoint and packet-provenance
theorems can be applied inside the step callback. -/
theorem runPolicies_replay_from_initial
    {runtime : GraphRuntime Player L Δ} {whole : Graph Player L Γ₀ Δ}
    {focal : Player} (players : Player → runtime.application.PlayerPolicy)
    (environment : runtime.application.EnvironmentPolicy)
    (schedule : List (@MessageApplication.Invocation Player))
    {leftInitial rightInitial leftFinal rightFinal : runtime.application.PolicyExecution}
    (initial : NativeReplayInvariant runtime whole focal leftInitial rightInitial)
    (leftSupported : leftFinal ∈
      (runtime.application.runPolicies players environment schedule leftInitial).support)
    (rightSupported : rightFinal ∈
      (runtime.application.runPolicies players environment schedule rightInitial).support)
    (step : ∀ (processed : List (@MessageApplication.Invocation Player)) invocation rest
      (leftCurrent rightCurrent leftNext rightNext : runtime.application.PolicyExecution),
      schedule = processed ++ invocation :: rest →
      NativeReplayInvariant runtime whole focal leftCurrent rightCurrent →
      leftCurrent ∈
        (runtime.application.runPolicies players environment processed leftInitial).support →
      rightCurrent ∈
        (runtime.application.runPolicies players environment processed rightInitial).support →
      leftNext ∈ (runtime.application.invoke players environment leftCurrent invocation).support →
      rightNext ∈
        (runtime.application.invoke players environment rightCurrent invocation).support →
      leftFinal ∈
      ...
      NativeReplayInvariant runtime whole focal leftNext rightNext) :
    NativeReplayInvariant runtime whole focal leftFinal rightFinal := by
  have replay : ∀ (processed remaining : List (@MessageApplication.Invocation Player))
      ...
      rightCurrent ∈
        (runtime.application.runPolicies players environment processed rightInitial).support →
      leftFinal ∈
        (runtime.application.runPolicies players environment remaining leftCurrent).support →
      rightFinal ∈
        (runtime.application.runPolicies players environment remaining rightCurrent).support →
      NativeReplayInvariant runtime whole focal leftFinal rightFinal := by
    intro processed remaining
    induction remaining generalizing processed with
    | nil =>
        intro leftCurrent rightCurrent split invariant _ _ leftTail rightTail
        simp only [MessageApplication.runPolicies, FinDist.mem_support_pure] at leftTail rightTail
        subst leftFinal
        subst rightFinal
        exact invariant
    | cons invocation rest ih =>
        ...
        have leftNextReached : leftNext ∈
            (runtime.application.runPolicies players environment
              (processed ++ [invocation]) leftInitial).support := by
...
/-- Replay a shorter actual prefix against the corresponding prefix of a
longer actual run, while exposing the longer run's future endpoint to every
step.  This is the form used to compare two reached actions whose invocation
indices are not assumed equal. -/
theorem runPolicies_replay_prefix_against_extension
    {runtime : GraphRuntime Player L Δ} {whole : Graph Player L Γ₀ Δ}
    {focal : Player} (players : Player → runtime.application.PlayerPolicy)
    (environment : runtime.application.EnvironmentPolicy)
    (common extra : List (@MessageApplication.Invocation Player))
    {leftInitial rightInitial leftBoundary rightFinal :
      runtime.application.PolicyExecution}
    (initial : NativeReplayInvariant runtime whole focal leftInitial rightInitial)
    (leftSupported : leftBoundary ∈
      (runtime.application.runPolicies players environment common leftInitial).support)
    (rightSupported : rightFinal ∈
      (runtime.application.runPolicies players environment (common ++ extra)
        rightInitial).support)
    (step : ∀ (processed : List (@MessageApplication.Invocation Player)) invocation rest
      (leftCurrent rightCurrent leftNext rightNext : runtime.application.PolicyExecution),
      common = processed ++ invocation :: rest →
      NativeReplayInvariant runtime whole focal leftCurrent rightCurrent →
      leftCurrent ∈
        (runtime.application.runPolicies players environment processed leftInitial).support →
      rightCurrent ∈
      ...
      leftBoundary ∈
        (runtime.application.runPolicies players environment rest leftNext).support →
      rightFinal ∈
        (runtime.application.runPolicies players environment (rest ++ extra) rightNext).support →
      NativeReplayInvariant runtime whole focal leftNext rightNext) :
    ∃ rightBoundary,
      rightBoundary ∈
        (runtime.application.runPolicies players environment common rightInitial).support ∧
      rightFinal ∈
        (runtime.application.runPolicies players environment extra rightBoundary).support ∧
      NativeReplayInvariant runtime whole focal leftBoundary rightBoundary := by
  rw [runtime.application.runPolicies_append] at rightSupported
  ...
  apply runPolicies_replay_from_initial players environment common initial leftSupported
    rightPrefix
  intro processed invocation rest leftCurrent rightCurrent leftNext rightNext split invariant
    leftReached rightReached leftInvoke rightInvoke leftResidual rightResidual
  apply step processed invocation rest leftCurrent rightCurrent leftNext rightNext split invariant
    leftReached rightReached leftInvoke rightInvoke leftResidual
  rw [runtime.application.runPolicies_append]
