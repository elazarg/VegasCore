...
theorem reactive_respond_progress (runtime : EventGraphRuntime graph) (inputs : graph.Inputs)
    ...
    State.ServiceProgress inputs 0 execution.application
      (execution.respond runtime.reactiveApplication who action).application := by
...
theorem reactive_resume_progress (runtime : EventGraphRuntime graph) (inputs : graph.Inputs)
    (players : Player → runtime.reactiveApplication.Policy) (actor : Option Player)
    (execution next : runtime.reactiveApplication.Execution)
    (invariant : execution.application.Invariant inputs)
    (reached : next ∈ (runtime.reactiveApplication.resume players actor execution).support) :
  ...
  | some who =>
      ...
      exact runtime.reactive_respond_progress inputs execution who action invariant

def reactiveTicks (runtime : EventGraphRuntime graph) : runtime.reactiveApplication.Command → Nat
  | .application .advanceClock => 1
  | _ => 0

theorem reactive_include_progress (runtime : EventGraphRuntime graph) (inputs : graph.Inputs)
    (execution : runtime.reactiveApplication.Execution) (id : MessageId Player)
    (invariant : execution.application.Invariant inputs) :
    State.ServiceProgress inputs 0 execution.application
      (execution.includePending runtime.reactiveApplication id).application := by
...
theorem reactive_environment_progress (runtime : EventGraphRuntime graph) (inputs : graph.Inputs)
    (execution next : runtime.reactiveApplication.Execution)
    (command : runtime.reactiveApplication.Command)
    (invariant : execution.application.Invariant inputs)
    (reached : next ∈ (execution.environmentStep runtime.reactiveApplication command).support) :
    State.ServiceProgress inputs (runtime.reactiveTicks command)
  ...
  | «include» id =>
      simp only [ReactiveApplication.Execution.environmentStep, FinDist.map_pure] at reached
      cases FinDist.mem_support_pure.mp reached
      exact runtime.reactive_include_progress inputs execution id invariant
  | application command =>
...
theorem reactive_dispatch_progress (runtime : EventGraphRuntime graph) (inputs : graph.Inputs)
    (players : Player → runtime.reactiveApplication.Policy)
    (execution next : runtime.reactiveApplication.Execution)
    (command : runtime.reactiveApplication.Command)
    (invariant : execution.application.Invariant inputs)
    (reached : next ∈ (runtime.reactiveApplication.dispatch players command execution).support) :
    State.ServiceProgress inputs (runtime.reactiveTicks command)
      execution.application next.application := by
  obtain ⟨middle, supported, moved⟩ := Set.mem_iUnion₂.mp (FinDist.support_bind .. ▸ reached)
  have environment := runtime.reactive_environment_progress inputs execution middle command
    invariant supported
  have response := runtime.reactive_resume_progress inputs players _ middle next
...
theorem interactionInstruction_ticks (runtime : EventGraphRuntime graph)
    (network : runtime.NetworkPolicy) (history : List runtime.reactiveApplication.EnvironmentEntry)
    (view : runtime.reactiveApplication.EnvironmentView) (instruction : ServiceInstruction graph)
    (command : runtime.reactiveApplication.Command)
    (supported : command ∈
      (runtime.interactionInstruction network history view instruction).support) :
    runtime.reactiveTicks command = instruction.ticks := by
...
theorem interactionStep_facts (runtime : EventGraphRuntime graph) (inputs : graph.Inputs)
    (players : Player → runtime.reactiveApplication.Policy) (network : runtime.NetworkPolicy)
    (instruction : ServiceInstruction graph)
    (execution next : runtime.reactiveApplication.Execution)
    (invariant : execution.application.Invariant inputs)
    (supported : next ∈ (runtime.interactionStep players network instruction execution).support) :
  ...
  have progress := runtime.reactive_dispatch_progress inputs players execution next command
    invariant moved
  rwa [runtime.interactionInstruction_ticks network _ _ instruction command selected] at progress

theorem runInteractionPlan_facts (runtime : EventGraphRuntime graph) (inputs : graph.Inputs)
    (players : Player → runtime.reactiveApplication.Policy) (network : runtime.NetworkPolicy)
    (plan : List (ServiceInstruction graph))
    (execution next : runtime.reactiveApplication.Execution)
    (invariant : execution.application.Invariant inputs)
    (supported : next ∈ (runtime.runInteractionPlan players network plan execution).support) :
  ...
  | cons instruction rest ih =>
      ...
      have first := runtime.interactionStep_facts inputs players network instruction
...
theorem runInteractionEpochs_facts (runtime : EventGraphRuntime graph) (inputs : graph.Inputs)
    (chosen : ServiceOrder graph) (networkTurns : Nat)
    (players : Player → runtime.reactiveApplication.Policy) (network : runtime.NetworkPolicy)
    (count : Nat) (execution next : runtime.reactiveApplication.Execution)
    (invariant : execution.application.Invariant inputs)
  ...
  | succ count ih =>
      obtain ⟨middle, moved, finished⟩ := Set.mem_iUnion₂.mp (FinDist.support_bind .. ▸ supported)
      have first := runtime.runInteractionPlan_facts inputs players network _
        execution middle invariant moved
