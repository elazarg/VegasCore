...
theorem runInteractionPlan_support_instruction (runtime : EventGraphRuntime graph)
    (players : Player → runtime.reactiveApplication.Policy) (network : runtime.NetworkPolicy)
    (before after : List (ServiceInstruction graph)) (instruction : ServiceInstruction graph)
    (execution final : runtime.reactiveApplication.Execution)
    (supported : final ∈ (runtime.runInteractionPlan players network
      (before ++ instruction :: after) execution).support) :
    ∃ prior ∈ (runtime.runInteractionPlan players network before execution).support,
      ∃ next ∈ (runtime.interactionStep players network instruction prior).support,
        final ∈ (runtime.runInteractionPlan players network after next).support := by
...
theorem reactive_application_support (runtime : EventGraphRuntime graph)
    (players : Player → runtime.reactiveApplication.Policy)
    (command : EnvironmentCommand graph) (execution next : runtime.reactiveApplication.Execution)
    (supported : next ∈ (runtime.reactiveApplication.dispatch players
      (.application command) execution).support) :
    next.application ∈ (environmentStep runtime execution.application command).support := by
  change next ∈ ((execution.environmentStep runtime.reactiveApplication
...
theorem interactionStep_sample_complete (runtime : EventGraphRuntime graph)
    (players : Player → runtime.reactiveApplication.Policy) (network : runtime.NetworkPolicy)
    (event : graph.EventId) (execution next : runtime.reactiveApplication.Execution)
    (ready : execution.application.config.cut.Ready event) (chance : graph.actor? event = none)
    (supported : next ∈
      (runtime.interactionStep players network (.sample event) execution).support) :
    event ∈ next.application.config.cut.completed := by
  have moved : next ∈ (runtime.reactiveApplication.dispatch players
  ...
  exact runtime.environmentStep_sample_complete _ _ event ready chance
    (runtime.reactive_application_support players _ execution next moved)
...
theorem interactionStep_expire_complete (runtime : EventGraphRuntime graph)
    ...
    (supported : next ∈
      (runtime.interactionStep players network (.expire event) execution).support) :
    event ∈ next.application.config.cut.completed := by
  have moved : next ∈ (runtime.reactiveApplication.dispatch players
...
theorem interactionEpoch_chance_complete (runtime : EventGraphRuntime graph) (inputs : graph.Inputs)
    (chosen : ServiceOrder graph) (networkTurns : Nat)
    (players : Player → runtime.reactiveApplication.Policy) (network : runtime.NetworkPolicy)
    (event : graph.EventId) (execution next : runtime.reactiveApplication.Execution)
    (invariant : execution.application.Invariant inputs)
    (ready : execution.application.config.cut.Ready event) (chance : graph.actor? event = none)
    (supported : next ∈ (runtime.runInteractionPlan players network
  ...
  obtain ⟨prior, priorMem, sampled, sampledMem, afterMem⟩ :=
    runtime.runInteractionPlan_support_instruction players network before after
      (.sample event) execution next supported
  have progress := runtime.runInteractionPlan_facts inputs players network before
    execution prior invariant priorMem
  have sampledProgress := runtime.interactionStep_facts inputs players network (.sample event)
    prior sampled progress.invariant sampledMem
  have suffix := runtime.runInteractionPlan_facts inputs players network after
  ...
  · exact suffix.completed (runtime.interactionStep_sample_complete players network event
...
theorem interactionEpoch_strategic_complete (runtime : EventGraphRuntime graph)
    ...
    (chosen : ServiceOrder graph) (networkTurns : Nat)
    (players : Player → runtime.reactiveApplication.Policy) (network : runtime.NetworkPolicy)
    (event : graph.EventId) (entered : Nat) (execution next : runtime.reactiveApplication.Execution)
    (invariant : execution.application.Invariant inputs)
    (ready : execution.application.config.cut.Ready event)
    (strategic : (graph.actor? event).isSome = true)
  ...
  have progress := runtime.runInteractionPlan_facts inputs players network before
    execution prior invariant priorMem
  have expiredProgress := runtime.interactionStep_facts inputs players network (.expire event)
  ...
  · apply suffix.completed
    apply runtime.interactionStep_expire_complete players network event prior expired
      priorReady strategic entered (progress.activated event entered activated priorReady.1)
      _ expiredMem
    rw [progress.clock, beforeTicks]
    exact dueAfterTick

def reactiveCompletionService (runtime : EventGraphRuntime graph) (chosen : ServiceOrder graph)
    (networkTurns : Nat) (players : Player → runtime.reactiveApplication.Policy)
    (network : runtime.NetworkPolicy) :
    CompletionService runtime runtime.reactiveApplication.Execution where
  state := ReactiveApplication.Execution.application
  epoch := runtime.runInteractionPlan players network (interactionEpoch chosen networkTurns)
  progress inputs before after invariant supported := by
    simpa only [interactionEpoch_ticks] using runtime.runInteractionPlan_facts inputs players
      network _ before after invariant supported
  chance inputs event before after := runtime.interactionEpoch_chance_complete inputs
    chosen networkTurns players network event before after
  due inputs event entered before after := runtime.interactionEpoch_strategic_complete inputs
...
theorem reactiveCompletionService_run (runtime : EventGraphRuntime graph)
    (chosen : ServiceOrder graph) (networkTurns : Nat)
    (players : Player → runtime.reactiveApplication.Policy) (network : runtime.NetworkPolicy)
    (count : Nat) (execution : runtime.reactiveApplication.Execution) :
    (runtime.reactiveCompletionService chosen networkTurns players network).run count execution =
      runtime.runInteractionEpochs chosen networkTurns players network count execution := by
...
theorem runInteractionEpochs_terminal (runtime : EventGraphRuntime graph) (inputs : graph.Inputs)
    (chosen : ServiceOrder graph) (networkTurns : Nat)
    (players : Player → runtime.reactiveApplication.Policy) (network : runtime.NetworkPolicy)
    (execution next : runtime.reactiveApplication.Execution)
    (invariant : execution.application.Invariant inputs)
    (supported : next ∈ (runtime.runInteractionEpochs chosen networkTurns players network
      runtime.serviceEpochs execution).support) : next.application.config.cut.Terminal := by
  apply (runtime.reactiveCompletionService chosen networkTurns players network).terminal
    inputs execution next invariant
  rwa [runtime.reactiveCompletionService_run]
...
theorem canonical_interaction_complete (runtime : EventGraphRuntime graph)
    ...
    (supported :
      ...
      let scheduler := runtime.interactionScheduler chosen networkTurns network
      let horizon := runtime.interactionHorizon chosen networkTurns
      result ∈ (((runtime.reactiveApplication.information
        initial horizon scheduler).runSingleMoverBehavioralFrom
          (runtime.reactiveApplication.singleMover initial horizon scheduler)
          (fun who => runtime.reactiveApplication.encodePolicy (players who)) (2 * horizon + 1)
  ...
  rw [runtime.canonical_interaction_service, FinDist.bind_map] at supported
  ...
  exact ⟨_, rfl, runtime.runInteractionEpochs_terminal input chosen networkTurns players network
