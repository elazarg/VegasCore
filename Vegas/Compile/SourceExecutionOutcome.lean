...
the same as projecting the coupled terminal source environment. Every result
...
theorem runPolicyNodes_observeSourceOutcome_eq_coupled [Fintype P]
  ...
  rw [observeSourceOutcome_of_terminal program out.current.graph hterminal]
  exact congrArg some (decodeSourceOutcome_coupled program.prog program.fresh state out)
