...
def decodeEventAction : {Γ : SourceCtx Player L} → {openNames : Finset VarId} →
  ...
  | _, _, .commit (payload := payload) name owner _ _ next, event, action =>
      Fin.cases
        (fun choice => some (.commit owner name payload
          ((BoundValue.resultEquiv _).symm choice)))
...
private theorem compileRankedNodes_actor {inputCount totalCount : Nat}
  ...
  | ret payoffs =>
      intro unique refs publications registry embedding refsBefore publicationsBefore index
      exact nomatch index
...
theorem eventOwner?_eq_actor {Γ : SourceCtx Player L} {openNames : Finset VarId}
  ...
  symm
  exact compileRankedNodes_actor program unique _ _ _ _ _ _ event
...
def compilePolicyTable {Field : Type} [DecidableEq Field]
  ...
  | _, _, .ret _, _, _, _, _, _, _, event, _, _ => nomatch event
  | _, _, .sample name fresh law next, unique, refs, publications, outputs,
  ...
  | _, _, .commit (payload := payload) name owner fresh guard next, unique, refs,
      publications, outputs, who, policy, event, store, history =>
      ...
      Fin.cases
        (fun store history =>
          if same : owner = who then
            match decodeObservation? who refs publications store with
            | some observation =>
                (policy.1 same (observation, history)).map (BoundValue.resultEquiv _)
  ...
  | Γ, _, .reveal (payload := payload) published owner name fresh selected unresolved
      next, unique, refs, publications, outputs, who, policy, event, store, history =>
...
the source kernel transported through the binding-result equivalence. -/
theorem compilePolicyTable_commit_of_decode {Field : Type} [DecidableEq Field]
    {layout : Field → Vegas.EventGraph.EventField Player L}
    {Γ : SourceCtx Player L} {openNames : Finset VarId}
    ...
    compilePolicyTable (.commit (payload := payload) name owner fresh guard next)
        unique refs publications outputs who policy
        ⟨0, Nat.zero_lt_succ (eventCount next)⟩ store history =
      (policy.1 same (observation, history)).map (BoundValue.resultEquiv _) := by
...
theorem compilePolicyTable_commit_of_decode_none {Field : Type} [DecidableEq Field]
    {layout : Field → Vegas.EventGraph.EventField Player L}
    {Γ : SourceCtx Player L} {openNames : Finset VarId}
    {name : VarId} {owner who : Player} {payload : L.Ty}
    {fresh : name ∉ Γ.map Prod.fst}
    ...
    (decoded : decodeObservation? who refs publications store = none) :
    compilePolicyTable (.commit (payload := payload) name owner fresh guard next)
        unique refs publications outputs who policy
        ⟨0, Nat.zero_lt_succ (eventCount next)⟩ store history =
...
theorem compilePolicyTable_reveal_of_decode {Field : Type} [DecidableEq Field]
    {layout : Field → Vegas.EventGraph.EventField Player L}
    {Γ : SourceCtx Player L} {openNames : Finset VarId}
...
theorem compilePolicyTable_reveal_of_decode_none {Field : Type} [DecidableEq Field]
    {layout : Field → Vegas.EventGraph.EventField Player L}
    {Γ : SourceCtx Player L} {openNames : Finset VarId}
    {published name : VarId} {owner who : Player} {payload : L.Ty}
    {fresh : published ∉ Γ.map Prod.fst}
