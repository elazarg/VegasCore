...
theorem binding_ordinary_inclusion
  ...
  have hnormal := runtime.blockEnvironment_normal roster polled.environmentHistory
    (State.environmentView runtime.application polled.native) (.bind timed)
    (by
      rw [hlength, Nat.mul_div_cancel]
      · exact hindex
      · omega) hactive
    (by rw [hlength]; exact Nat.mul_mod_left _ _)
  have hcommand : runtime.blockEnvironment roster polled.environmentHistory
    ...
    simp only [ApplicationImage.serviceCommand, MessageApplication.latestSubmissionCommand,
      WindowedApplication.eraseEnvironmentView, State.environmentView]
    change FinDist.pure (runtime.liftEnvironmentCommand
      (match polled.native.pool.nextSerial owner with
        | 0 => .wait
        | serial + 1 => if (polled.native.pool.lookup (owner, serial)).isSome
          then .include (owner, serial) else .wait)) = _
