...
extracted root policy. Its chosen value leads by the remaining source steps
...
theorem BlockSourceStep.exists_focal_source_choice
  ...
  | @binding pending name owner ty guard tail newName accounted fresh state unrestricted
      ...
      obtain ⟨site, hroot⟩ := root_kernel_of_suffix hinitial horigins hroster howners command hpure
        hrelay hrelayOther trace (.here guard tail) _ _ (by
          simpa [sourcePolicyCheckpointsFrom,
            SourcePolicyCheckpoints.ownedCommit, head] using hlocal')
      refine ⟨name, ty, guard, tail, site, value, unrestricted _ _, rfl, hroot, ?_⟩
      rw [hsource]
      exact .refl _
  | @publicChoice pending name publicName owner ty guard tail newName unresolved accounted
      ...
      obtain ⟨site, hroot⟩ := root_kernel_of_suffix hinitial horigins hroster howners command hpure
        hrelay hrelayOther trace (.here guard _) _ _ (by
          simpa [sourcePolicyCheckpointsFrom,
            SourcePolicyCheckpoints.ownedCommit, head, witness, publicDecisionCheckpoints]
            using hlocal)
      refine ⟨name, ty, guard, _, site, value, legal, rfl, hroot, ?_⟩
      rw [hsource]
      ...
      obtain ⟨site, hroot⟩ := root_kernel_of_suffix hinitial horigins hroster howners command hpure
        hrelay hrelayOther trace (.here guard _) _ _ (by
          simpa [sourcePolicyCheckpointsFrom,
            SourcePolicyCheckpoints.ownedCommit, head, witness, publicDecisionCheckpoints]
            using hlocal)
      refine ⟨name, ty, guard, _, site, spec.encoding.symm result, legal, rfl, hroot, ?_⟩
      rw [hsource]
      exact .single (.reveal .here _)
