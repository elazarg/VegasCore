...
theorem currentNodeOptionLaw_support_some
  ...
  exact
    Projected.currentNodeValueLaw_support_some
      program presentation semantics σ h hterm replay hnode fuel hsupport
      hactive
      (by
        simpa [currentNodeOptionLaw,
  ...
  have hvalueSupport' :
      some value ∈
        (Math.ProbabilityMassFunction.pushforward
          ...
          project).support := by
    simpa [currentNodeOptionLaw, compiled, view, project] using hvalueSupport
...
noncomputable def currentSourceCommitValueLaw
  ...
  let optionLaw :=
    currentNodeOptionLaw program presentation semantics
      σ h hterm (who := who) node
  let htotal :
      ∀ result ∈ optionLaw.support,
        ∃ value, result = some value :=
    fun result hresult =>
      currentNodeOptionLaw_support_some
        program presentation semantics σ h hterm replay hnode fuel
    ...
    valueLaw.bindOnSupport fun value hvalueSupport =>
      ...
      have hguard :
        ...
        simpa [sourceValue] using
          sourceLegal_of_currentNodeOptionLaw_support
            program presentation semantics σ h hterm replay hnode fuel
            hsupport value
