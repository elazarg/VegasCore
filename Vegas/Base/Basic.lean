...
/-!
# Shared Vegas base interface

This file records the generic value, context, visibility, environment, and
payoff infrastructure on which both `VegasCore` syntax and event graphs are
built.

## Architecture

The file separates **the embedded language** (`IExpr`) from
visibility-aware contexts (`VCtx`), with **erasure** as the bridge between
typed Vegas contexts and plain expression-language contexts.
