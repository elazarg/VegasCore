...
contexts and SSA-named variables.
...
- **SSA**: Enforced separately by `Vegas.Protocol.WF`.
- **Semantics**: Defined in `Vegas.BigStep` and `Vegas.TraceSemantics`.
-/
...
def exprLanguage : Vegas.ExprLanguage where
  Ty := BaseTy
...
abbrev BindTy : Type := Vegas.VisBindTy Player exprLanguage
abbrev Ctx : Type := Vegas.VisCtx Player exprLanguage
abbrev HasVar : Ctx → VarId → BindTy → Type :=
  Vegas.VisHasVar (Player := Player) (L := exprLanguage)
abbrev Env (Γ : Ctx) : Type := Vegas.VisEnv (Player := Player) exprLanguage Γ

namespace BindTy

abbrev base : BindTy → BaseTy := Vegas.VisBindTy.base
...
@[simp] theorem cons_get_there {Γ : Ctx} {x y : VarId} {τ σ : BindTy}
  ...
  exact (Vegas.VisEnv.cons_get_there
    (Player := Player) (L := exprLanguage) (Γ := Γ) (x := x) (y := y)
    (τ := τ) (σ := σ) (v := v) (env := env) (h := h))

abbrev toView (p : Player) (env : Env Γ) :
    Env (Vegas.viewCtx (Player := Player) (L := exprLanguage) p Γ) :=
  Vegas.VisEnv.toView (Player := Player) (L := exprLanguage) p env

abbrev toPub (env : Env Γ) :
    Env (Vegas.pubCtx (Player := Player) (L := exprLanguage) Γ) :=
  Vegas.VisEnv.toPub (Player := Player) (L := exprLanguage) env

abbrev toFlat {Γ : Ctx} (env : Env Γ) :
    Env (Vegas.flattenCtx (Player := Player) (L := exprLanguage) Γ) :=
  Vegas.VisEnv.toFlat (Player := Player) (L := exprLanguage) env

abbrev toFlatView (p : Player) {Γ : Ctx} (env : Env Γ) :
    Env (Vegas.flattenCtx (Player := Player) (L := exprLanguage)
      (Vegas.viewCtx (Player := Player) (L := exprLanguage) p Γ)) :=
  Vegas.VisEnv.toFlatView (Player := Player) (L := exprLanguage) p env
...
abbrev canSee (p : Player) : BindTy → Bool :=
  Vegas.canSee (Player := Player) (L := exprLanguage) p

abbrev viewCtx (p : Player) : Ctx → Ctx :=
  Vegas.viewCtx (Player := Player) (L := exprLanguage) p

abbrev pubCtx : Ctx → Ctx :=
  Vegas.pubCtx (Player := Player) (L := exprLanguage)

abbrev flattenCtx : Ctx → Ctx :=
  Vegas.flattenCtx (Player := Player) (L := exprLanguage)

namespace HasVar

abbrev ofViewCtx {p : Player} :
    HasVar (viewCtx p Γ) x τ → HasVar Γ x τ :=
  Vegas.VisHasVar.ofViewCtx (Player := Player) (L := exprLanguage) (p := p)

abbrev ofPubCtx : HasVar (pubCtx Γ) x τ → HasVar Γ x τ :=
  Vegas.VisHasVar.ofPubCtx (Player := Player) (L := exprLanguage)

abbrev ofPubToView {p : Player} :
    HasVar (pubCtx Γ) x τ → HasVar (viewCtx p Γ) x τ :=
  Vegas.VisHasVar.ofPubToView (Player := Player) (L := exprLanguage) (p := p)

abbrev toFlatten : HasVar Γ x τ → HasVar (flattenCtx Γ) x (.pub τ.base) :=
  Vegas.VisHasVar.toFlatten (Player := Player) (L := exprLanguage)

abbrev unflatten {Γ : Ctx} {x : VarId} {b : BaseTy} :
    HasVar (flattenCtx Γ) x (.pub b) →
    (τ : BindTy) × HasVar Γ x τ × PLift (τ.base = b) :=
  Vegas.VisHasVar.unflatten (Player := Player) (L := exprLanguage)
...
theorem flattenCtx_map_fst :
  ...
  simpa [flattenCtx] using
    (Vegas.flattenCtx_map_fst (Player := Player) (L := exprLanguage) (Γ := Γ))

theorem flattenCtx_length :
    (flattenCtx Γ).length = Γ.length := by
  simpa [flattenCtx] using
    (Vegas.flattenCtx_length (Player := Player) (L := exprLanguage) (Γ := Γ))

theorem flattenCtx_idempotent :
    flattenCtx (flattenCtx Γ) = flattenCtx Γ := by
  simpa [flattenCtx] using
    (Vegas.flattenCtx_idempotent (Player := Player) (L := exprLanguage) (Γ := Γ))
...
instance exprKit : Vegas.VisExprKit Player exprLanguage where
  Expr := Expr
  eval := @evalExpr
  deps := @exprVars
...
visibility-aware expression interface. -/
noncomputable instance distKit : Vegas.VisDistKit Player exprLanguage where
  DistExpr := DistExpr
  eval := @evalDistExpr
  deps := @distExprVars
...
abbrev SampleMode : BindTy → Type := Vegas.SampleMode (Player := Player) (L := exprLanguage)

abbrev distCtx (τ : BindTy) (m : SampleMode τ) (Γ : Ctx) : Ctx :=
  Vegas.distCtx (Player := Player) (L := exprLanguage) τ m Γ

def Env.projectDist {Γ : Ctx} (τ : BindTy) (m : SampleMode τ)
    (env : Env Γ) : Env (distCtx τ m Γ) :=
  Vegas.VisEnv.projectDist (Player := Player) (L := exprLanguage) τ m env
...
visibility-aware expression interface. -/
noncomputable instance payoffKit : Vegas.VisPayoffKit Player exprLanguage where
...
abbrev evalR {Γ : Ctx} {b : BaseTy} {who : Player} {x : VarId}
  ...
  Vegas.evalGuard (Player := Player) (L := exprLanguage) inferInstance R a view

abbrev Prog : Ctx → Type := Vegas.Prog Player exprLanguage
...
abbrev CommitKernel (who : Player) (Γ : Ctx) (b : BaseTy) : Type :=
  Vegas.CommitKernel Player exprLanguage who Γ b

abbrev Profile : Type := Vegas.Profile Player exprLanguage

-- ============================================================================
-- § 9. Partial profiles
-- ============================================================================

abbrev PProfile : Type := Vegas.PProfile Player exprLanguage
