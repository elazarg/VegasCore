...
For the candidate host, `candidate_approximate_nash_iff_of_source_floor`
also permits a profile-local pointwise source floor against the fixed opponents,
while retaining a uniform source cap on quitting settlements. No utility bound
on executions generated by different opponent policies is required for that floor.
