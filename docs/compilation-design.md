...
The public-message implementation separates carrier types from operational
semantics. `MessageInterface` fixes application state, payload, private-command,
environment-command, and player/environment observation types. The native
states, actions, histories, and policy types depend on that interface.
`MessageApplication` supplies transitions and observation functions. Replacing
an interpreter component while retaining the interface therefore retains the
same strategy and outcome carriers; whether their laws agree is a separate
theorem. A carrier-preserving handler extension can be conservative for traffic
generated by reference policies while admitting additional behavior from other
policies. Changing wire representations or observation types instead requires
an explicit translation; the interface split supplies no such correctness claim.
