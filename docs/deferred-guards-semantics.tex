...
This note specifies deferred guards and their failure-aware source execution.
Sections \ref{sec:publications}--\ref{sec:invariant} describe the standalone
\lean{Interaction.GuardedPublication} component, whose publication slots carry
a runtime three-valued status. The Vegas source language (section
\ref{sec:source}) no longer uses that component: because its syntax fixes
which reveal publishes each private cell, each guard is checked exactly once,
at the reveal completing it, with a binary verdict and no runtime status. Both
are formalized in Lean. The compiler consumes the \lean{SourceProgram}
semantics and has checked exact outcome and unilateral-deviation laws through
the ordered graph to its ideal pending-message runtime.
...
and publication results. A binding of type $A$ is a $\mathsf{Result}(A)$: an
ordinary $a:A$, or $\mathsf{fail}$ for an unopenable candidate. It is distinct
from the public cell its reveal creates. A published alias has type
\[
...
\begin{itemize}
\item \lean{Initial.revealed}: every private cell is revealed by the end of the
