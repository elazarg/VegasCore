...
For finite observed outcomes, the [incentive criterion](spe-incentive-criterion.md)
characterizes preservation for all utilities exactly: every target continuation
comparison must lie in the closed cone generated by source comparisons for the
same player. This is weaker than separately matching prescribed and deviated
laws. The criterion and an abstract off-path disclosure impossibility are
checked; the reactive compiler's full continuation certificate remains open.
...
An ideal public-history service enforces the rule and combines it with uniform,
at-most-once selection. A ledger realization and the positive SPE theorem remain
open. The
